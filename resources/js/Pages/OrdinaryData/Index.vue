...
async function removeData(id) {
    loading.value = true;
    let start = new Date().getTime();
    router.delete('/ordinary-data/' + id, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
            loading.value = false;
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Ordinary Data</h2>
        </template>
                ...
                <div class="bg-white dark:bg-gray-800 p-4 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        ...
                        <div class="px-6 pt-4 pb-8  flex gap-4 justify-between items-center bg-white dark:bg-gray-900">
                            ...
                            <Link :href="route('ordinary-data.create')">
                            <button type="button"
                            ...
                            <tbody>
                                <tr v-for="(item, index) in props.data.data" :key="index"
                                    ...
                                    <td class="px-6 py-4 flex gap-2">
                                        <Link :href="route('ordinary-data.edit', {
                                            id: item.id,
